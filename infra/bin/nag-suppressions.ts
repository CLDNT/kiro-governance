  ...
  NagSuppressions.addStackSuppressions(
    ...
    [
      ...
      },
      {
        ...
        reason:
          'Remaining wildcards are scoped to service-required patterns that cannot be enumerated at synth time: CloudWatch Logs on `/aws/lambda/*` (per-function log groups), EC2 ENI actions on `*` (mandatory for Lambda-in-VPC / RDS IAM auth), SSM `GetParameter` on the non-secret `/kiro-governance/config/*` and `/deliverpro/*` config prefixes, KMS `Decrypt` conditioned on `kms:ViaService = ssm.<region>.amazonaws.com`, and S3 on the `evidence/*` object prefix. The Bedrock agent execution role uses `bedrock:InvokeModel[WithResponseStream]` on `foundation-model/*` (all regions — cross-region inference profiles fan out across regions) and `inference-profile/*` in this account, and the analysis Lambda uses `bedrock:InvokeAgent` on `agent/*`; the concrete agent/model IDs are only known post-deploy, so these are the tightest static patterns available. Each per-credential SSM secret (Slack bot/provisioning tokens, GitHub read-token) is granted as a SINGLE parameter ARN via SsmSecretGrant — those are NOT wildcarded. The RDS IAM `rds-db:connect` ARNs are pinned to the exact dbuser (`kiro_mcp_app` / `kiro_phase2`).',
      },
