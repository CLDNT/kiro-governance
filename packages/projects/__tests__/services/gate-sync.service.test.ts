 ...
 * Asserts: correct { matched, resolved, skipped }; idempotent re-run (resolved:0); unmapped gate
 * and missing checkpoint → skipped; already-resolved → skipped; unlinked → all zero; unknown
...
function makePool(opts: {
  ...
  const query = jest.fn(async (sql: string, params: unknown[] = []) => {
    ...
    if (s.startsWith('SELECT jira_key, github_repo, github_url FROM projects')) {
      ...
      return {
        rows: [{ jira_key: params[0], github_repo: 'repo', github_url: 'https://github.com/acme/repo' }],
        rowCount: 1,
      };
    }
    ...
    if (s.startsWith('SELECT 1 FROM macro_checkpoints')) {
      const checkpointName = params[1] as string;
      const cp = state.get(checkpointName);
      return { rows: cp?.exists ? [{ ok: 1 }] : [], rowCount: cp?.exists ? 1 : 0 };
    }
...
const auditCall = (calls: Recorded[]) => calls.find((c) => c.sql.trim().startsWith('INSERT INTO project_link_audit'));
const updateCalls = (calls: Recorded[]) => calls.filter((c) => c.sql.trim().startsWith('UPDATE macro_checkpoints'));

const MARKDOWN = [
  '- [x] 1.4 SRS approved',
  '- [x] Design docs approved by Faraz',
  '- [x] spec file approved', // alias → 'Spec strategy approved' (UNMAPPED → skipped)
...
describe('syncGatesFromRepo — counting & apply', () => {
  it('resolves mapped+existing gates, skips unmapped and missing-row gates', async () => {
    const { calls } = makePool({
      checkpoints: {
        'Working SRS reviewed by SA': { exists: true, reached: false },
        'Technically validate 6 design docs with spec strategy by SA': { exists: true, reached: false },
    ...
    // SRS + Design docs → matched+resolved. Spec strategy → unmapped skip. Code → missing-row skip.
    expect(summary).toEqual({ project_id: 'DP-001', matched: 2, resolved: 2, skipped: 2 });
  ...
  it('CR16-H2: writes ONE append-only project_link_audit row when resolved>0 (actor + source + gates)', async () => {
    const { calls } = makePool({
      checkpoints: { 'Working SRS reviewed by SA': { exists: true, reached: false } },
    });
    ...
    expect(source).toEqual({ owner: 'acme', repo: 'repo', content_ref: 'W/"etag123"' });
  ...
  it('re-run over already-resolved checkpoints → resolved:0, matched preserved, NO audit row', async () => {
    const { calls } = makePool({
      checkpoints: {
        ...
        'Technically validate 6 design docs with spec strategy by SA': { exists: true, reached: true },
      },
    });

    const summary = await syncGatesFromRepo('DP-001', 'sub-admin');

    // SRS + Design docs exist-but-already-reached → matched + skipped. Spec strategy unmapped +
    // Code missing → skipped. resolved must be 0.
...
describe('syncGatesFromRepo — no-op & error paths', () => {
  it('unlinked project (fetch returns null content) → all zero, no UPDATE', async () => {
    mockFetchProgressFile.mockResolvedValue({ content: null, reason: 'not_linked' });
    const { calls } = makePool({ checkpoints: {} });

    const summary = await syncGatesFromRepo('DP-001', 'sub-admin');
    expect(summary).toEqual({ project_id: 'DP-001', matched: 0, resolved: 0, skipped: 0 });
    expect(updateCalls(calls)).toHaveLength(0);
  });

  it('owner_not_allowed (CR16-H1 fail-closed) → all zero, no UPDATE', async () => {
    mockFetchProgressFile.mockResolvedValue({ content: null, reason: 'owner_not_allowed', owner: 'evil', repo: 'repo' });
    const { calls } = makePool({ checkpoints: {} });
  ...
  it('unknown project → NotFoundError (mapped to 404 by the handler)', async () => {
    ...
    await expect(syncGatesFromRepo('NOPE-999', 'sub-admin')).rejects.toMatchObject({ code: 'NOT_FOUND' });
  });
  ...
  it('propagates a GithubFetchError (rate-limit) so the handler can 503', async () => {
    ...
    mockFetchProgressFile.mockRejectedValue(new GithubFetchError('GITHUB_RATE_LIMITED', 'rate limited'));
    makePool({ checkpoints: {} });

    await expect(syncGatesFromRepo('DP-001', 'sub-admin')).rejects.toMatchObject({ code: 'GITHUB_RATE_LIMITED' });
  });
});
